  ...
  return (
    <>
      ...
      <div className="row my-3">
        ...
        { images.map((img) => {
          return <ImageItem key={ img._id } img={ img } id={ setid } edit={ setedit } title={ settitle } desc={ setdesc } tag={ settag } url={ seturl } update={ getimages } showadd={setshowadditem} />
        }) }

{(edit || showadditem)  && <Edt showedit={setedit} additem={showadditem} settitle={settitle}  setdesc={setdesc} settag={settag} seturl={seturl} edithandler={handleSubmitedit} title={title} 
desc={desc}
tag={tag}
url={url} 
